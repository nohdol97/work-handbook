...
업무 지식을 분야와 역할이 바뀌어도 계속 확장하는 한영 핸드북이다. 학습 원문을 개념별 지식으로 통합하고 설계·운영·문제 해결에 다시 사용한다. 현재 학습 자료 2건을 반입했으며 데이터 플랫폼 기술·구조·커리큘럼 23쌍, 실무 프롬프트 모음 18쌍, 홈·지식 관리 방법·용어집을 합쳐 한영 44쌍이다. 후속 자료의 AI 평가·Databricks·Snowflake·플랫폼 비교·운영·최종 구조를 통합해 Phase 1~21 개념 학습 범위를 제공한다. 실제 구축·장애 실험 완료를 뜻하지 않는다.

## 실무 프롬프트

[프롬프트 모음](docs/ko/prompts/index.md)에 새 실무 예시 100개를 주제별로 정리했다. 개념 문서 23개 예시를 합쳐 123개 모두 한국어·English 탭으로 바꾸고 여러 줄로 읽고 복사할 수 있다. 문서 전체 언어는 상단 메뉴로 선택한다. 예시는 기존 학습 내용에서 파생한 템플릿이며 실제 모델 실행·운영 성과를 뜻하지 않는다.
...
`make validate`는 단위 테스트, 하네스 구조, 한영·출처·검토 증거, Mermaid 구문, strict 빌드, 프롬프트 언어 쌍·줄바꿈·문서 간 일치와 HTML 링크·언어 전환·게시 범위를 검사한다. 의미 검토는 에이전트/사람이 수행하고 자동화는 해시가 최신인지 검사한다. 외부 링크 가용성, 실제 기술 주장, 예제 운영 적합성은 별도 증거가 필요하다.

원문에 공백 정규화 기록이 있으면 `check_handbook.py`가 ledger를 복원한 byte 수와 SHA-256도 확인한다. 불일치는 업로드와 보존본을 대조해 원인을 찾고, 검사를 통과시키기 위해 hash만 바꾸지 않는다. 일반 source 파일의 기존 추적 규칙은 유지한다.
