...
업무 지식을 분야와 역할이 바뀌어도 계속 확장하는 한영 핸드북이다. 학습 원문을 개념별 지식으로 통합하고 설계·운영·문제 해결에 다시 사용한다. 현재 학습 자료 1건을 반입했으며 데이터 플랫폼 기술·구조·커리큘럼 18쌍, 실무 프롬프트 모음 18쌍, 홈·지식 관리 방법·용어집을 합쳐 한영 39쌍이다. 자료의 Phase 1~15와 16.1은 개념 학습 범위이며, 미학습 후속 과정은 목차로 유지한다.

## 실무 프롬프트

[프롬프트 모음](docs/ko/prompts/index.md)에 새 실무 예시 100개를 주제별로 정리했다. 기존 17개 예시를 포함해 117개 모두 한국어·English 탭으로 바꾸고 여러 줄로 읽고 복사할 수 있다. 문서 전체 언어는 상단 메뉴로 선택한다. 예시는 기존 학습 내용에서 파생한 템플릿이며 실제 모델 실행·운영 성과를 뜻하지 않는다.
