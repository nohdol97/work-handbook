...
This is a professional knowledge handbook and its personal maintenance harness. Handbook content lives at this root. Optional independent product repositories live under `project/`. Read `handbook-contract.md` before every source import or documentation update.
...
## 11. Handbook-specific completion

Published content is bilingual under `docs/ko` and `docs/en`; English handbook pages are an explicit exception to Korean-only control-document writing. Follow all preservation, extraction, semantic review and privacy obligations in `handbook-contract.md`.
Run `make validate` before delivery and `make finalize` for the configured local vault. No final success without byte-verified Markdown mirror. Do not fabricate studied content when no source exists. Keep machine-local vault configuration ignored. Public Pages activation requires immediate confirmation.
