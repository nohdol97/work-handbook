...
Each scenario has Situation; Context to Give the LLM; Example Prompt in a closed text fence; Expected Output; What the LLM Can Get Wrong; How to Validate. Prompts use simple English, specific inputs and outputs, and separate observations, assumptions, constraints, hypotheses, missing evidence and next checks. Do not redesign before assessing a design; do not confuse correlation with causation.
...
8. Stage named paths, inspect sanitized diff, commit and push verified work to private source repo. Pages workflow exists but activation/publication requires immediate confirmation. Once approved and enabled, main pushes validate/build/deploy.
